...
5. Provide proper proper`idDoc (type`, `docNumber`and `issuer)` based on the sub-client’s address provided in the item 4. For addresses in Australia either driver’s license or passport are accepted. For non-Australian addresses only passport is accepted as a document type. Also, this should be a foreign passport ID, not local.
{% endhint %}
