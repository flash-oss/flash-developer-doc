...
A sub-client is created and has the `INITIATED` status.\
At this stage, the request to activate a sub-client has been received and the system is currently processing it.
...
From this status it would eventually transition to either `ACTIVE` or `FAILED_KYC`. Typically, we can't disclose the exact reason of the KYC failure, while it mostly happens because we do not have risk appetite for your client.
